...
metrics <- read.csv("data/configuration-table.csv", encoding = "latin1") |>
  select(
    "metric",
    "domain"
  )
...
abbreviate_domain_metric_year <- function(
  vec,
  str_length,
  met,
  min_yr,
...
insert_superscript <- function(main_string, insert_string) {
  # The position from the end where the insertion should start
  # 12 characters from the end means the insertion point is 12 characters BEFORE the end.
  end_position <- 12

  n <- nchar(main_string)

  start_index <- n - end_position + 1
  # start_index is 33. The insertion happens after the 32nd character.

  part_a <- substr(main_string, 1, start_index - 1)
  # "This is a string (truely) - from ("

  part_c <- substr(main_string, start_index, n)
  # "2015-2020)"

  # Combine the parts
  new_string <- paste0(part_a, insert_string, part_c)
  return(new_string)
}

main_plot_data <- tibble::tribble(
  ...
  mutate(
    metric = case_when(
      Domain == "Performance" ~ abbreviate_domain_metric_year(
        Domain,
        4,
        Metric,
        Minimum.year,
      ...
      .default = abbreviate_domain_metric_year(
        Domain,
        3,
        Metric,
        Minimum.year,
  ...
  mutate(
    across(
      .cols = all_of(
        c("min", "first_quartile", "mean", "median", "third_quartile", "max")
      ),
  ...
  mutate(
    across(
      .cols = all_of(
        c("min", "first_quartile", "third_quartile", "max")
      ),
    ...
    postscript = as.integer(cumsum(!is.na(final_label))),
    postscript = case_when(
      is.na(final_label) ~ "",
      .default = paste0("[", postscript, "]")
    ),
    metric = insert_superscript(metric, postscript)
  )

# label_chem_base <- function(x) {
#   x <- gsub("([A-Za-z])([0-9]+)", "\\1[\\2]", x)
#   x <- gsub("\\^(\\d*[+-])", '^("\\1")', x)
#   # parse() returns an expression vector; no lists needed
#   parse(text = x)
# }
...
# ) +
# scale_x_discrete(labels = label_chem_base)
